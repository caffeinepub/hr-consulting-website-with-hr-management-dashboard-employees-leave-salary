...
export default function HrEmployeePayslipsPage() {
  ...
  const employeeIdBigInt = employeeId ? BigInt(employeeId) : BigInt(0);

  const { data: employee, isLoading: employeeLoading } = useGetEmployee(employeeIdBigInt);
  const { data: payslips = [], isLoading: payslipsLoading } = useGetEmployeePayslips(employeeIdBigInt);
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        <Button
          variant="ghost"
          size="icon"
          onClick={() => navigate({ to: '/hr/employees/$employeeId', params: { employeeId: employeeId || '0' } })}
        >
